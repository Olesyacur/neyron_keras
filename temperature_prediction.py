...
# Функция-генератор, возвр-щая временные посл-ти образцов и их целей
def generator(
        data, lookback, delay,
        min_index, max_index,
...
# Функции-генераторы, возвращ-щие обучающие, проверочные и контрол. данные
lookback = 1440
step = 6
delay = 144
batch_size =128
...
# Сколько разнужно обратиться к val_gen, чтобы получ. пров-й набор целиком
val_steps = (300000 - 200001 - lookback) // batch_size

# сколько раз нужно обрат-ся к test_gen, чтобы получ. контр-й набор целиком
...
'''Базовое решение с привлечением машинного обучения.'''

# Обучение и оценка полноценной связи
model = Sequential()
...
# Обучение и оценка модели на основе GPU
model = Sequential()
...
# Обучение и оценка модели на основе GPU с регуляризацией прореживанием
model = Sequential()
...
# Обучение и оценка модели с несколькими слоями GPU и с регуляриз. прореж.
model = Sequential()
model.add(layers.GRU(32,
                    dropout= 0.1,
